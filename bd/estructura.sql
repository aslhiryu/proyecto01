...
$$
BEGIN
	IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename='tipo_tarjeta')
	THEN
        CREATE TABLE tipo_tarjeta(
            id_tipo_tarjeta     VARCHAR(36) NOT NULL,
            nombre              VARCHAR(25) NOT NULL,
            CONSTRAINT PK_tipoTarjeta PRIMARY KEY(id_tipo_tarjeta)
        );
	END IF;

	IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename='pais')
	THEN
        CREATE TABLE pais(
	...
	IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename='estado_tarjeta')
	THEN
        CREATE TABLE estado_tarjeta(
            id_estado_tarjeta   VARCHAR(36) NOT NULL,
            nombre              VARCHAR(30) NOT NULL,
            CONSTRAINT PK_estadoTarjeta PRIMARY KEY(id_estado_tarjeta)
        );
	END IF;
